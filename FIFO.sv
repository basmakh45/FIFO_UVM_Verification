...
always @(posedge clk or negedge rst_n) begin
	if (!rst_n) begin
		...
		underflow <= 0;
       		...
       		underflow <= (empty && rd_en) ? 1 : 0;  //if trying to read and is empty then underflow =1
    	end
end
	...
	if (!rst_n) begin
		...
		if	( ({wr_en, rd_en} == 2'b10) && !full) 
			...
			count <= count - 1;
            		...
            		count <= count + 1;   // only write happens
        	else if (({wr_en, rd_en} == 2'b11) && full)
            		count <= count - 1;   // only read happens
	end
end
...
assign almostfull = (count == FIFO_DEPTH-1)? 1 : 0; //fix
